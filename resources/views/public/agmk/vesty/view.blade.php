<x-public-layout>
    ...
    @php
        $vesty_date  = date_parse($vesty->created_at ?? '');
        $all_images = getAllImages($vesty, true);
    @endphp
                    ...
                    <div class="col-lg-8">
                        <div class="news-view__main" id="newsPrint">
                            <div class="news-view__title">
                                <h3>{{ _t($vesty->title) }}</h3>

                                <div class="news-inner__item__date">
                                    <span>
                                        {{ ($vesty_date['day'] < 10 ? '0'.$vesty_date['day'] : $vesty_date['day']) }}
                                        {{ getMonthName($vesty->created_at) }}
                                        {{ $vesty_date['year'] }},
                                        {{ str_pad($vesty_date['hour'], 2, '0', STR_PAD_LEFT) }}:{{ str_pad($vesty_date['minute'], 2, '0', STR_PAD_LEFT) }}
                                    </span>
                                    <span>
                                        <i class="icon-view"></i>{{ $vesty->views }}
                                    </span>
                                </div>
                            </div>
                                ...
                                <div class="news-view__image">
                                    <div class="owl-carousel owl-theme" id="news-carousel">
                                        @foreach ($all_images as $file)
                                            @php
                                                $imgUrl = url_u() . 'vesty/' . ltrim($file->url, '/');
                                            @endphp
                                            <div class="item">
                                                <div class="news-view__image">
