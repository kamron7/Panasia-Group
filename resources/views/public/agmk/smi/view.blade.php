<x-public-layout>
    ...
    @php
        $smi_date  = date_parse($smi->created_at ?? '');
        $all_images = getAllImages($smi, true);
    @endphp
                            ...
                            <div class="news-view__title">
                                ...
                                <div class="news-inner__item__date">
                                    <span>
                                        ...
                                        {{ getMonthName($smi->created_at) }}
                                        {{ $smi_date['year'] }},
                                        {{ str_pad($smi_date['hour'], 2, '0', STR_PAD_LEFT) }}:{{ str_pad($smi_date['minute'], 2, '0', STR_PAD_LEFT) }}
                                    </span>
                                    <span>
                                        <i class="icon-view"></i>{{ $smi->views }}
                                    </span>
                                </div>
                            </div>
                                ...
                                <div class="news-view__image">
                                    <div class="owl-carousel owl-theme" id="news-carousel">
                                        @foreach ($all_images as $file)
                                            @php
                                                $imgUrl = url_u() . 'smi/' . ltrim($file->url, '/');
                                            @endphp
                                            <div class="item">
                                                <div class="news-view__image">
